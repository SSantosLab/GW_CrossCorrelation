...
import BCCsims_PreProcessing_zerr as NK_PreProcessing
import Plotting_zerr as Plotting
...
if (os.path.isfile(gw_dir) and os.path.isfile(cat_dir)):
    ...
    # write function to find appropiate galaxy catalog in directory for given GW catalog. Example:
    # file = gw_dir
...
else: # case where both are directories
    # write function to relate them here. Example:
    # for r, di, f in os.walk(gw_dir):
    #     for file in f:
...
for gw_file, cat_file in zip(gw_files, cat_files):
    ...
    z_pdf = GW_info[0]
    z_weights_pdf = GW_info[1]
    lumd_pdf = GW_info[2]
    weights_pdf = GW_info[3]
    cat_ra = GW_info[4]
    cat_dec = GW_info[5]
    gw_prob = GW_info[6]
    NSIDE = GW_info[7]
    GW_ID = GW_info[8]
    pixels = GW_info[9]
    gw_distances = GW_info[10]
    distsigma = GW_info[11]
    distnorm = GW_info[12]
    completeness = GW_info[13]
    ...
    if (completeness > min_completeness and not_skip): # additionally, event will be skipped if min_completeness is not reached
       # create all appropiate subdirectories for event
        ...
        NK_GW.run(h0_values, z_pdf, z_weights_pdf, lumd_pdf, weights_pdf, cat_ra, cat_dec, gw_prob, NSIDE, outdir_event, outdir_all, GW_ID, gw_distances, distsigma, nest,  pixels)
        ...
        if do_randoms:
            ...
            NK_GW.run_randoms(h0_values, z_pdf, z_weights_pdf, lumd_pdf, weights_pdf, cat_ra, cat_dec, gw_prob, NSIDE, outdir_randoms, outdir_all, GW_ID, gw_distances, distsigma, nest,  pixels)
